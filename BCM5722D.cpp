...
static const struct SupportedDevice
{
  ...
  const char *model;
    ...
    { DEVICEID_BCM5761E, "BCM5761E NetLink (TM) Gigabit Ethernet" },  { 0, NULL }
};
...
void BCM5722D::systemWillShutdown(IOOptionBits specifier)
{
...
UInt32 BCM5722D::outputPacket(mbuf_t m, void *param)
{
    ...
    if (txFreeSlot == 0) {
        DebugLog("Output queue stalled");
        queueStalled = true;
        return kIOReturnOutputStall;
    }
    
    segmentCount = txCursor->getPhysicalSegmentsWithCoalesce(m, segments, kTxMaxSegmentCount);
    ...
    if (segmentCount <= txFreeSlot) {
        ...
        OSAddAtomic16(-segmentCount, &txFreeSlot);
        txProducerIdx = producer;
        BUMP_NETSTAT(outputPackets);
        
        status = kIOReturnOutputSuccess;
    } else {
        DebugLog("%u -> %u segments, %u free, consumer index: %u", (unsigned int)mbuf_pkthdr_len(m), (unsigned int)segmentCount, txFreeSlot, txLocalConsumerIdx);
        queueStalled = true;
